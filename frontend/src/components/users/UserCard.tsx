...
function UserCard({
  ...
  const skills = [
    { label: skillLabels.skill1, value: user.skill1 },
    { label: skillLabels.skill2, value: user.skill2 },
    { label: skillLabels.skill3, value: user.skill3 },
    { label: skillLabels.skill4, value: user.skill4 },
  ];
      ...
      <div className="user-card-header">
        ...
        <div className="user-card-title">
          ...
          <div className="user-card-meta">
            {showPickOrder && user.pickOrder != null && (
              <span>Pick #{user.pickOrder}</span>
            )}
            {showRole && user.role && <span>{getRoleLabel(user.role)}</span>}
            {showUsername && user.username && <span>@{user.username}</span>}
            {showTeam && user.assignedTeamName && (
              <span>{user.assignedTeamName}</span>
            )}
          </div>
        </div>
      </div>
            ...
            return (
              <div className="user-card-skill" key={skill.label}>
                <div className="user-card-skill-header">
                  <span className="user-card-skill-name">{skill.label}</span>
                  <strong className="user-card-skill-value">
                    {skill.value ?? "-"}
                  </strong>
                </div>
